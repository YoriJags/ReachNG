...
VerticalType = Literal["real_estate", "recruitment", "events", "fintech", "legal", "logistics", "agriculture", "agency_sales", "small_business", "b2b_saas"]
...
async def run_campaign(body: RunCampaignRequest, background_tasks: BackgroundTasks):
    ...
    # Background for: large batches, or any live HITL run (enrichment + Mongo upserts
    # can exceed Railway's 60s edge timeout even for small batches).
    # Dry runs are always synchronous so the curl response shows the drafted messages.
    if not body.dry_run and (body.max_contacts > 30 or body.hitl_mode):
        ...
        return {
            ...
            "message": f"Campaign running in background ({body.max_contacts} contacts, hitl={body.hitl_mode}). Check dashboard for queued drafts.",
            "discovery": {},
        }
