...
async def _handle_image_attachment(
    ...
    try:
        ...
        # ── EYO Shield — flag suspect transfers before the owner releases goods.
        # Best-effort, non-blocking: warns the owner, never auto-declines, and
        # never interrupts the normal HITL acknowledgement queued below.
        try:
            from services.shield import assess_transfer, shield_alert_text
            db = _db()
            cdoc = match.client_doc
            if not cdoc and client_id:
